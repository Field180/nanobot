...
| ~~TD-X~~ | WAL maintenance monitoring blind spot during idle + initial periods | R17→R18 | R17: staleness alert at >600s. R18: **initial-phase blind spot eliminated** — when maintenance has never succeeded but uptime >600s, `/health/detailed` emits "maintenance never completed" warning. Prometheus gauge reports process uptime (not -1) when never succeeded, enabling `NanobotWalMaintenanceStale` to fire during initial phase. | Source: r17_* + r18_health_never_completed_alert + r18_prom_uptime_fallback |

...
| ~~TD-AA~~ | No Grafana dashboard — monitoring requires manual PromQL construction | R20→R21 | `deploy/grafana/nanobot-dashboard.json` provides 10-panel importable dashboard. R21: **PromQL metric name validity enforced** — CI test extracts all `nanobot_*` metric names from dashboard expressions and cross-validates against `prometheus_metrics()` source. Dashboard will break the build if referencing renamed/removed metrics. | File: r20_dashboard_* ; Sync: r21_dashboard_metrics_valid |
| ~~TD-AB~~ | Runbook/alerts.yml drift risk — no sync enforcement | R21 | Bidirectional CI sync: (1) every alert in `alerts.yml` must appear in `RUNBOOK.md`, (2) RUNBOOK must not document phantom alerts absent from `alerts.yml`, (3) all metric names in `alerts.yml` expressions validated against actual export. Prevents silent documentation drift. | Sync: r21_alerts_runbook_sync + r21_runbook_no_phantom_alerts + r21_alerts_metrics_valid |
| ~~TD-AC~~ | Audit tests lack CI enforcement — failures silent under `unittest discover` | R22 | `test_audit_fixes.py` uses custom `check()`/`sys.exit(1)` which only fires when run standalone (`__name__ == "__main__"`). R22: Makefile `test-audit` target runs it as standalone script, wired into both `make ci` and `make check` gates. Pre-commit hook (`make install-hooks`) inherits this via `make check`. Build now hard-fails on any audit test regression. | Makefile: `test-audit` target; Gate: `ci` and `check` depend on `test-audit` |

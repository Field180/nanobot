...
| ~~TD-AC~~ | Audit tests lack CI enforcement — failures silent under `unittest discover` | R22→R23 | R22: Makefile `test-audit` target (local gate). R23: **Remote CI via `.github/workflows/audit.yml`** — runs on every push/PR to master/main. Executes `test_audit_fixes.py` standalone + unit tests + safety-check. Unbypassable: developer cannot merge without passing. Eliminates the "developer discipline" dependency that local-only gates suffer from. | Makefile: `test-audit`; Remote: `.github/workflows/audit.yml` |

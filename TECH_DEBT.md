...
| ~~TD-U~~ | WAL checkpoint effectiveness unknown — PASSIVE may silently fail with active readers | R13→R14 | `wal_checkpoint(PASSIVE)` return value inspected: `(busy, log, checkpointed)`. If `log > 0` and `checkpointed == 0`, tracked as ineffective. R14 upgrade: `/health/detailed` alert requires **both** `ineffective_count ≥ 5` AND WAL size grew >20% from baseline (filters benign read-lock contention). Alert suppressed for 5 minutes after firing to prevent flooding. WAL baseline recorded at streak start, cleared on success. | Source: r14_tracks_wal_baseline + r14_clears_baseline_on_success + r14_has_suppress_state; AST: r14_health_checks_wal_growth + r14_health_has_suppression |
| ~~TD-V~~ | shell_execute unblocked command volume invisible | R13→R14 | `_SHELL_PASSED` counter + `/health/detailed` now exposes `shell` component with `total`, `passed`, `blocked`, `errors`, `block_ratio`. Ratio enables external monitoring to detect anomalous bypass patterns. | Source: r13_metrics_has_passed + r13_record_increments_passed; AST: r14_health_has_shell_component + r14_health_shell_passed |

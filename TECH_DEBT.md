...
| ~~TD-W~~ | No Prometheus alert rules template — monitoring requires manual setup | R16→R17 | `deploy/prometheus/alerts.yml` provides 7 pre-built rules (R16: 6 original + R17: `NanobotWalMaintenanceStale`). Staleness rule fires when `nanobot_wal_maint_last_check_seconds > 600` for 2m, covering idle-period monitoring gaps. All rules have explicit `for:` durations. | File: r16_alerts_yml_exists + r17_alerts_has_stale_rule + r17_alerts_stale_uses_metric |
| ~~TD-X~~ | WAL maintenance monitoring blind spot during idle periods | R17 | `/health/detailed` exposes `maintenance_last_check_seconds` (seconds since last success, null if never). Staleness alert at >600s. Prometheus gauge `nanobot_wal_maint_last_check_seconds` per DB (-1 if never run). | Source: r17_health_has_maint_age + r17_health_staleness_alert + r17_prom_maint_last_check |

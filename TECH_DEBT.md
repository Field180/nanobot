...
| ~~TD-U~~ | WAL checkpoint effectiveness unknown — PASSIVE may silently fail with active readers | R13→R15 | `wal_checkpoint(PASSIVE)` return value inspected. R14: growth-aware alert (count≥5 AND WAL grew >20%). R15: baseline refreshed every `_DB_MAINT_INEFFECTIVE_THRESHOLD` ineffective runs (prevents stale baseline during long streaks). WAL >50MB triggers **critical** alert that bypasses suppression entirely. Prometheus exports: `nanobot_wal_size_bytes`, `nanobot_wal_maint_fail_count`, `nanobot_wal_maint_ineffective_count` per DB. | Source: r15_baseline_refresh_periodic; AST: r15_health_wal_critical_50mb + r15_prom_wal_size + r15_prom_wal_ineffective + r15_prom_wal_fail |

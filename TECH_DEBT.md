...
| ~~TD-W~~ | No Prometheus alert rules template — monitoring requires manual setup | R16 | `deploy/prometheus/alerts.yml` provides 6 pre-built rules: `NanobotWalSizeCritical` (>50MB), `NanobotWalSizeWarning` (>10MB), `NanobotWalMaintenanceFailing`, `NanobotWalCheckpointIneffective`, `NanobotShellBlockRatioLow` (<1%), `NanobotShellErrorRateHigh` (>30%). Thresholds are documented as adjustable. | File: r16_alerts_yml_exists + r16_alerts_has_wal_size + r16_alerts_has_ineffective + r16_alerts_has_shell_ratio |
